...
import { storage, updateUserStatusDocument } from '../../config/firebase';
      ...
      <DashboardUserWrapper>
        {user?.additionalData?.devis ? (
          <Title>
            ...
            <Body>
              Votre conseiller MeChauffer vous contactera très bientôt pour vous accompagner dans votre projet
            </Body>
          </Title>
        ) : (
          <Title>
        ...
        <WorkWrapper>
          {user?.additionalData?.devis &&
            user?.additionalData?.devis[4]?.value?.map((type, index) => (
              <>
                {Array.isArray(type.value) &&
                  type.value.map((typeDetail, indexDetail) => (
                    <>
                      <Flex key={index}>
                        <Type>
                          <H4 fontSize="1.8">{type.name}</H4>
                          <Body fontSize="1.4" color={'rgba(55, 73, 87, 0.4)'}>
                            {typeDetail.name}
                          </Body>
                          <WrapperTraitement>
                            <Body fontSize="1.4">
                              Afin d`accélérer le traitement de votre dossier, nous vous invitons à télécharger les
                              documents requis en cliquant sur le bouton ci-dessous.
                            </Body>
                          </WrapperTraitement>
                        </Type>
                        <Green>
                          <GreenWrapper>
                            ...
                            <ValidationDoc>
                              <Body fontSize="1.4" color={themeContext.colors.primary}>
                                Estimation Prime MeChauffer
                              </Body>
                              <Body fontSize="1.2" color={'rgba(55, 73, 87, 0.4)'}>
                                Documents en cours de validation
                              </Body>
                            </ValidationDoc>
                            <Body lineHeight="1.5" fontSize="1.2" color={themeContext.colors.white}>
                              {' '}
                              Attention : ne signez pas votre devis avant d’avoir fait votre demande de prime par
                              téléphone auprès d’un conseiller MeChauffer
                            </Body>
                          </GreenWrapper>
                        </Green>
                      </Flex>
                  ...
                  <Flex key={index}>
                    ...
                    <Green>
                      <GreenWrapper>
                        ...
                        <Body lineHeight="1.5" fontSize="1.2" color={themeContext.colors.white}>
                          {' '}
                          Attention : ne signez pas votre devis avant d’avoir fait votre demande de prime par téléphone
                          auprès d’un conseiller MeChauffer
                        </Body>
                      </GreenWrapper>
                    </Green>
                  </Flex>
          ...
          <ButtonWrapper>
            <ButtonPrimary
              ...
              bgColor={themeContext.colors.white}
              hoverBgColor={themeContext.colors.primary}
              hoverColor={themeContext.colors.white}
              disabled={false}
            >
              Je télécharge mes documents
            </ButtonPrimary>
          </ButtonWrapper>
          <ModalContent userUid={user.uid} open={open} onCloseModal={onCloseModal} />
        </WorkWrapper>
        ...
        <ButtonPrimary
          type="submit"
          width={'20rem'}
          bgColor={themeContext.colors.logout}
          hoverBgColor={themeContext.colors.logoutHover}
          hoverColor={themeContext.colors.white}
          disabled={false}
...
export const ModalContent = ({ userUid, open, onCloseModal, getDevisByUserIdFct }) => {
  const { user } = useAuth();
  ...
  const handleChange = (event, filename) => {
    ...
    Array.from(event.target.files).map((file, index) => {
      const storageRef = ref(storage, `/${userUid}/${filename}${index}.png`);
      ...
      uploadTask.on(
        ...
        () => {
          getDownloadURL(uploadTask.snapshot.ref).then(async (url) => {
            if (filename === 'ci') {
              if (impot === true) {
                await updateUserStatusDocument(userUid, 'validation');
                if (user.additionalData.role === 'parrain') getDevisByUserIdFct();
              }
              setCi(true);
  ...
  useEffect(() => {
    const listRef = ref(storage, `/${userUid}`);
    setCi(false);
    setImpot(false);
    ...
    listAll(listRef)
      .then((res) => {
        ...
        res.items.forEach((itemRef) => {
          if (itemRef.name.includes('ci')) setCi(true);
          else if (itemRef.name.includes('impot')) setImpot(true);
  ...
  const deleteFile = (filename) => {
    ...
    const desertRef = ref(storage, `/${userUid}/${filename}0.png`);

    // Delete the file
    deleteObject(desertRef)
      .then(() => {
        if (filename === 'ci') setCi(false);
        else if (filename === 'impot') setImpot(false);
    ...
    // Create a reference to the file to delete
    const desertRef1 = ref(storage, `/${userUid}/${filename}1.png`);
