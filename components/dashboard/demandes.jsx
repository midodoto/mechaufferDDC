...
import { ModalContent } from '../dashboard-user/index.jsx';
...
export const DemandesList = ({ demandes, getDevisByUserIdFct }) => {
  const themeContext = useContext(ThemeContext);
  const [open, setOpen] = useState(false);

  const onOpenModal = (userUid) => setOpen(userUid);
  const onCloseModal = () => setOpen(false);
  return (
    <Wrapper>
      {demandes?.length > 0 && (
        <Notice>
          Bénéficiez de 2 XCP supplémentaires en renseignant les informations du bénéficiaire pour votre première
          demande
        </Notice>
      )}
      {/* <Grid>
    ...
    {demandes.map((demande) => {
      ...
      {demandes?.length > 0 ? (
        <table>
          ...
          {demandes?.map((demande, index) => {
            ...
            var date = moment(demande?.data?.additionalData?.date);
            return (
              <tr key={index}>
                <td className="demande">
                  <Body1 fontSize="1.4">
                    {demande?.data?.additionalData.lastname} {demande?.data?.additionalData.firstname}
                  </Body1>
                </td>
                {demande?.data?.additionalData.status === 'new' ? (
                  <td className="demande">
                    <FlexStatus>
                      <Status className="orange"></Status>
                      <Body1 fontSize="1.4">En attente du devis</Body1>
                    </FlexStatus>
                  </td>
                ) : demande?.data?.additionalData.status === 'document' ? (
                  <td className="demande">
                    <FlexStatus>
                      <Status className="orange"></Status>
                      <Body1 fontSize="1.4">En attente des documents</Body1>
                    </FlexStatus>
                  </td>
                ) : demande?.data?.additionalData.status === 'validation' ? (
                  <td className="demande">
                    <FlexStatus>
                      <Status className="orange"></Status>
                      <Body1 fontSize="1.4">En attente de validation</Body1>
                    </FlexStatus>
                  </td>
                ) : demande?.data?.additionalData.status === 'done' ? (
                  <td className="demande">
                    <FlexStatus>
                      <Status className="green"></Status>
                      <Body1 fontSize="1.4">Validée</Body1>
                    </FlexStatus>
                  </td>
                ) : (
                  <td className="demande">
                    <FlexStatus>
                      <Status className="red"></Status>
                      <Body1 fontSize="1.4">Refusée</Body1>
                    </FlexStatus>
                  </td>
                )}
                <td className="demande">
                  <Body1 fontSize="1.4">{date.format('DD MMMM, YYYY')}</Body1>
                </td>
                {demande?.data?.additionalData.status === 'new' ? (
                  <td className="demande">
                  ...
                  <td className="demande">
                    <WrapperDemandeButton>
                      <ButtonPrimary
                        onClick={() => onOpenModal(demande.uid)}
                        bgColor={themeContext.colors.primary}
                        hoverBgColor={themeContext.colors.primary}
                        hoverColor={themeContext.colors.white}
                      >
                        Je renseigne les documents
                      </ButtonPrimary>
                    </WrapperDemandeButton>
                  </td>
                ) : demande?.data?.additionalData.status === 'validation' ? (
                  <td className="demande">
                    <WrapperDemandeButton></WrapperDemandeButton>
                  </td>
                ) : demande?.data?.additionalData.status === 'done' ? (
                  <td className="demande">
                    <WrapperDemandeButton></WrapperDemandeButton>{' '}
        ...
        <Empty>
          <span>
            <Body3>Vous n’avez pas encore de demande en cours.</Body3>
            <Body3> Cliquez sur le bouton “Créer une nouvelle demande” pour en créer une.</Body3>
          </span>
          <Link href={'/devis'}>
            <ButtonPrimary
              bgColor={themeContext.colors.primary}
      ...
      <ModalContent
        ...
        getDevisByUserIdFct={getDevisByUserIdFct}
      />
    </Wrapper>
  );
};

const Demandes = ({ demandes, getDevisByUserIdFct }) => {
  const { user } = useAuth();
  // const [demandes, setDemandes] = useState(null);

  // const getDevisByUserIdFct = async () => {
  //   const d = await getDevisByUserId(user.uid);
  //   if (user?.additionalData?.user) d.push({ uid: '', data: { additionalData: user?.additionalData?.user } });
  //   setDemandes(d);
  // };
  ...
  return (
    <DemandesStyle>
      <Flex>
        ...
        <ContentHeader>
          <Body fontSize="1.6">
            Réduction des émissions de CO2 & gaz à effet de serre, et économies réalisées par opération
          </Body>
          <ul>
            <li>
