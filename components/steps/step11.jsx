...
const Step11 = ({ display, setStep, initialUser }) => {
  const { user, signup, removeParrainFirstUserState } = useAuth();
  ...
  const handleSignup = async (e) => {
    ...
    try {
      if (data.password === data.confirmPassword) {
        ...
        await removeParrainFirstUserState(user);
        setLoading(false);
        TokenClean();
        InitialState();
        router.replace('/devis', undefined, { shallow: true });
        ...
        <Wrapper>
          ...
          <Step2 display={displayStep === 2}>
            ...
            <Body2 className={'subtitle'}>
              Si oui, merci de renseigner son numero de parainage. Dans le cas contraire, cliquez sur “Continuer”.
            </Body2>
            <InputWrapper>
              <input
                type="text"
