...
const ExtraStep = ({ setStep, OverwriteDevis, devisReducer, cards }) => {
  ...
  const [value, setValue] = useState({
    title: devisReducer.data && devisReducer.data[3] ? devisReducer.data[3].value[1] : null,
  });
      ...
      {cards && cards.extra && (
        <>
          ...
          <ButtonWrapper>
            <ButtonPrimary
              onClick={() => {
                const val = devisReducer.data[3].value;
                val.push(value.title);
                OverwriteDevis({ step: 4, data: { key: 'Type de chauffage', value: val } });
                setStep(5);
              }}
              width={'26rem'}
...
const Step4 = ({ display, setStep, extraStepChauffage, setExtraStepChauffage }) => {
  ...
  const [value, setValue] = useState({
    title: devisReducer.data && devisReducer.data[3] ? devisReducer.data[3].value[0] : null,
  });
  ...
  return (
    <Step4Style display={display}>
            ...
            onClick={() => {
              if (cards.find((element) => value && element.title === value.title)?.extra?.length > 0) {
                ...
                OverwriteDevis({
                  step: 3,
                  multipleStep: true,
                  data: { key: 'Type de chauffage', value: [value.title] },
                });
              } else {
      ...
      <StepWrapper display={extraStepChauffage === 2}>
        <ExtraStep
          setStep={setStep}
          setExtraStepChauffage={setExtraStepChauffage}
          OverwriteDevis={OverwriteDevis}
          devisReducer={devisReducer}
          cards={cards.find((element) => value && element.title === value.title)}
        />
      </StepWrapper>
    </Step4Style>
  );
