...
const ExtraStep = ({ setExtraStep, extraStep, OverwriteDevis, devisReducer, cards, setMultipleStep, multipleStep }) => {
  ...
  useEffect(() => {
    if (
      multipleStep2 === 1 &&
      ((value.extra && value.extra.length === 0) || (card[0].type === 'number' && card[0].extra.length === 0))
    ) {
      setMultipleStep2(0);
...
const Step5 = ({ display, setStep }) => {
  ...
  const cards = [
    {
      ...
      extra: [
        ...
        },
        {
          ...
          subtitle:
            'Cela nous suffit pour estimer votre prime, un artisan prendra les mesures exactes lors de sa visite',
          extra: [
            {
              image: '/images/cards/house.svg',
  ...
  return (
    <Step5Style display={display}>
      ...
      {value.map((val, index) => {
        return (
          <StepWrapper display={extraStep - 1 === index} key={index}>
            <TypeOfWork
              setStep={setStep}
              setExtraStep={setExtraStep}
              extraStep={extraStep}
